/*******************************************************************************************
*
*   RTextJP - Japanese-friendly UTF-8 text helpers for raylib
*
*   Copyright (c) 2026 LuisR385
*   SPDX-License-Identifier: MIT
*
*   RTextJPはraylib用の非公式な日本語テキスト補助ライブラリです。
*   本プロジェクトはraylib公式配布の一部ではありません。
*
...
*     インクルードしてください。それ以外の .c ファイルでは通常どおりインクルードします。
*
*       #define RTEXTJP_IMPLEMENTATION
*       #include "rtextjp.h"
*
*     すべての公開関数を翻訳単位内だけで使う場合は、RTEXTJP_STATIC も定義できます。
*
*   English:
*     Define RTEXTJP_IMPLEMENTATION in exactly one .c file before including this header.
*     Define RTEXTJP_STATIC as well to give all public functions internal linkage.
*
*   This software is designed for C99 and raylib 6.0 or later.
*
*******************************************************************************************/
...
#define RTEXTJP_VERSION       "0.1.0"
            ...
            #define RTEXTJPAPI static __attribute__((unused))
        #else
            #define RTEXTJPAPI static
        #endif
    #else
        #define RTEXTJPAPI extern
    #endif
#endif
...
/** \file rtextjp.h
 *  \brief raylib用の日本語対応UTF-8テキスト描画・入力補助。
 *
 *  RTextJPはC99のシングルヘッダライブラリです。日本語を空白なしで折り返し、
 *  行頭・行末の禁則処理を行います。英語の単語単位の折り返しにも対応します。
 */

/** \brief 横方向の文字揃え。 */
typedef enum RTextJPAlign {
    RTEXTJP_ALIGN_LEFT = 0,   /**< 左揃え。 */
    RTEXTJP_ALIGN_CENTER,     /**< 中央揃え。 */
    RTEXTJP_ALIGN_RIGHT       /**< 右揃え。 */
} RTextJPAlign;

/** \brief 矩形内の縦方向の文字揃え。 */
typedef enum RTextJPVerticalAlign {
    RTEXTJP_VALIGN_TOP = 0,   /**< 上揃え。 */
    RTEXTJP_VALIGN_MIDDLE,    /**< 中央揃え。 */
    RTEXTJP_VALIGN_BOTTOM     /**< 下揃え。 */
} RTextJPVerticalAlign;

/** \brief 縦書きの列が進む方向。 */
typedef enum RTextJPVerticalDirection {
    RTEXTJP_VERTICAL_RIGHT_TO_LEFT = 0, /**< 日本語で一般的な右から左。 */
    RTEXTJP_VERTICAL_LEFT_TO_RIGHT      /**< 左から右。 */
} RTextJPVerticalDirection;

/**
 * \brief フォントへ含める日本語文字集合。値はビット単位で組み合わせられます。
 *
 * CUSTOM は追加文字列だけを使う最小構成です。どの値でも表示可能ASCIIと全角空白は
 * 自動追加されます。JIS_X_0208 は非漢字、第1水準、第2水準の組み合わせです。
 *
 * \par English
 * Bit flags for predefined Japanese font character sets. Printable ASCII and U+3000 are
 * always included; custom UTF-8 characters can be added separately.
 */
typedef enum RTextJPCharacterSet {
    RTEXTJP_CHARACTER_SET_CUSTOM = 0,                /**< 追加文字列のみ（ASCIIは自動追加）。 */
    RTEXTJP_CHARACTER_SET_JIS_X_0208_KANA = 1 << 0, /**< JIS X 0208の平仮名・片仮名。 */
    RTEXTJP_CHARACTER_SET_JIS_X_0208_NON_KANJI = 1 << 1, /**< 記号・英数字・かな等。 */
    RTEXTJP_CHARACTER_SET_JIS_X_0208_LEVEL_1 = 1 << 2, /**< 第1水準漢字 2,965字。 */
    RTEXTJP_CHARACTER_SET_JIS_X_0208_LEVEL_2 = 1 << 3, /**< 第2水準漢字 3,390字。 */
    RTEXTJP_CHARACTER_SET_JIS_X_0208 =
        RTEXTJP_CHARACTER_SET_JIS_X_0208_NON_KANJI |
        RTEXTJP_CHARACTER_SET_JIS_X_0208_LEVEL_1 |
        RTEXTJP_CHARACTER_SET_JIS_X_0208_LEVEL_2 /**< JIS X 0208の6,879文字。 */
} RTextJPCharacterSet;

/**
 * \brief テキスト描画の共通スタイル。
 *
 * 初期値は必ず RTextJPStyleDefault() で作成し、変更したいメンバーだけを書き換えます。
 * \code{.c}
 ...
 *
 * \par English
 * Common drawing style. Initialize it with RTextJPStyleDefault().
 */
typedef struct RTextJPStyle {
    Font font;                          /**< 使用するraylibのフォント。 */
    float fontSize;                     /**< 表示サイズ（ピクセル）。 */
    float spacing;                      /**< 文字間隔（ピクセル）。 */
    float lineSpacing;                  /**< 行間隔（ピクセル）。 */
    Color color;                        /**< 文字色。 */
    RTextJPAlign alignment;               /**< 横方向の揃え。 */
    RTextJPVerticalAlign verticalAlignment; /**< 矩形内の縦方向の揃え。 */
    bool wrap;                          /**< trueなら幅に合わせて折り返す。 */
    bool kinsoku;                       /**< trueなら簡易禁則処理を行う。 */
    bool clip;                          /**< trueなら描画を矩形内に切り抜く。 */
} RTextJPStyle;

/**
 * \brief レイアウトを測定した結果。
 * \par English
 * Result returned by the measurement functions.
 */
typedef struct RTextJPMetrics {
    Vector2 size;       /**< 実際に使用する幅と高さ。 */
    int lineCount;      /**< 横書きでは行数、縦書きでは列数。 */
    int codepointCount; /**< Unicodeコードポイント数。 */
} RTextJPMetrics;

/**
 * \brief 1行入力欄の状態。文字列のメモリは呼び出し側が所有します。
 *
 * RTextJPInputInit() で初期化してください。cursor と length はUTF-8のバイト位置です。
 * 初心者はこれらを直接変更せず、用意された関数を使用してください。
 *
 * \par English
 * State for a single-line, fixed-buffer UTF-8 input field.
 */
typedef struct RTextJPInput {
    char *text;     /**< 呼び出し側が用意したUTF-8バッファ。 */
    int capacity;   /**< 終端NULを含むバッファ容量（バイト）。 */
    int length;     /**< 終端NULを含まない使用量（バイト）。 */
    int cursor;     /**< カーソルのバイト位置。 */
    bool active;    /**< trueの間だけキーボード入力を受け付ける。 */
} RTextJPInput;

/**
 * \brief 入力欄の枠とカーソルの見た目。
 * \par English
 * Visual style for RTextJPDrawInputBox().
 */
typedef struct RTextJPInputStyle {
    Color backgroundColor;   /**< 背景色。 */
    Color borderColor;       /**< 非アクティブ時の枠色。 */
    Color activeBorderColor; /**< アクティブ時の枠色。 */
    Color placeholderColor;  /**< プレースホルダーの文字色。 */
    Color cursorColor;       /**< カーソルの色。 */
    float padding;           /**< 枠から文字までの余白。 */
    float borderWidth;       /**< 枠線の太さ。 */
} RTextJPInputStyle;

/**
 * \brief 初心者向けの標準テキストスタイルを返します。
 * \param font 使用するフォント。
 * \param fontSize 表示サイズ（ピクセル）。
 * \param color 文字色。
 * \return 折り返し・禁則処理・クリップが有効なスタイル。
 * \par English
 * Returns a beginner-friendly default style.
 */
RTEXTJPAPI RTextJPStyle RTextJPStyleDefault(Font font, float fontSize, Color color);

/**
 * \brief 標準の入力欄スタイルを返します。
 * \par English
 * Returns the default input-box appearance.
 */
RTEXTJPAPI RTextJPInputStyle RTextJPInputStyleDefault(void);

/**
 * \brief UTF-8の先頭1文字をUnicodeコードポイントへ変換します。
 * \param text 読み取り元。NUL終端文字列の途中を渡せます。
 * \param bytes 読み取ったバイト数の格納先。NULでは0、不正な並びでは1です。NULL可。
 * \return コードポイント。不正な並びは U+FFFD、終端は0です。
 * \par English
 * Decodes one UTF-8 codepoint. Invalid input produces U+FFFD and consumes one byte.
 */
RTEXTJPAPI int RTextJPUtf8Decode(const char *text, int *bytes);

/**
 * \brief UnicodeコードポイントをUTF-8へ変換します。
 * \param codepoint 変換するコードポイント。
 * \param output 5バイト以上の配列。結果の後ろにNULを書き込みます。
 * \return 書き込んだバイト数。不正なコードポイントは0です。
 * \par English
 * Encodes one codepoint into a NUL-terminated UTF-8 byte sequence.
 */
RTEXTJPAPI int RTextJPUtf8Encode(int codepoint, char output[5]);

/**
 * \brief 文字列が正しいUTF-8か調べます。
 * \param text NUL終端文字列。NULLは不正として扱います。
 * \return 正しければtrue。
 * \par English
 * Returns true when the complete string is valid UTF-8.
 */
RTEXTJPAPI bool RTextJPUtf8IsValid(const char *text);

/**
 * \brief UTF-8文字列のコードポイント数を数えます。
 *
 * 不正な1バイトも置換文字1個として数えるため、常に前へ進みます。
 * \par English
 * Counts codepoints; each invalid byte is counted as one replacement character.
 */
RTEXTJPAPI int RTextJPUtf8CodepointCount(const char *text);

/**
 * \brief コードポイント番号をUTF-8のバイト位置へ変換します。
 * \param text NUL終端文字列。
 * \param codepointIndex 0始まりの位置。範囲外は文字列末尾へ丸めます。
 * \return 文字列先頭からのバイト位置。
 * \par English
 * Converts a codepoint index to a byte offset, clamped to the string.
 */
RTEXTJPAPI int RTextJPUtf8ByteIndex(const char *text, int codepointIndex);

/**
 * \brief プリセットと追加文字列から、実際に読み込む重複なしの文字数を返します。
 *
 * 表示可能ASCIIと全角空白を含む数です。customCharactersはNULLでも構いません。
 * メモリ確保に失敗した場合は0を返します。
 * \par English
 * Returns the unique glyph count for a preset/custom combination, including automatic ASCII.
 */
RTEXTJPAPI int RTextJPCharacterSetCodepointCount(RTextJPCharacterSet characterSet,
                                             const char *customCharacters);

/**
 * \brief 日本語文字集合プリセットと任意の追加文字からフォントを読み込みます。
 *
 * characterSetはビットORで組み合わせられます。customCharactersには、プリセットに
 * ない固有名詞、絵文字、記号などをUTF-8で渡せます。NULLなら追加しません。
 * 大きな集合はフォントアトラスの作成時間とVRAMを増やすため、必要な最小集合を選んでください。
 *
 * \par English
 * Loads a font from combinable Japanese presets plus optional custom UTF-8 characters.
 */
RTEXTJPAPI Font RTextJPLoadFontFromCharacterSet(const char *fileName, int baseSize,
                                            RTextJPCharacterSet characterSet,
                                            const char *customCharacters);

/**
 * \brief 実際に表示する文字から、ASCIIも含むフォントを読み込みます。
 *
 * \param fileName TTFまたはOTFフォントへのパス。
 * \param baseSize フォントアトラスを作る基準サイズ。
 * \param characters アプリで表示する全文字を連結したUTF-8文字列。
 * \return raylibのFont。失敗時は IsFontValid() がfalseになります。
 *
 * 同じ文字は自動的に1個へまとめられ、ASCII U+0020〜U+007Eは自動追加されます。
 * 返されたフォントは、不要になったとき UnloadFont() してください。
 *
 * \warning charactersに含めなかった日本語文字は、フォントの代替グリフで表示されます。
 * 入力欄では、入力される可能性のある文字もcharactersへ含めてください。
 *
 * \par English
 * Loads a font containing printable ASCII plus the unique characters found in characters.
 */
RTEXTJPAPI Font RTextJPLoadFontFromText(const char *fileName, int baseSize, const char *characters);

/**
 * \brief 横書きテキストの必要サイズを測定します。
 * \param text UTF-8文字列。
 * \param maxWidth 折り返す最大幅。0以下では自動折り返しません。
 * \param style 描画と同じスタイル。
 * \return 必要サイズ、行数、コードポイント数。
 * \par English
 * Measures horizontal text with the same wrapping rules as RTextJPDrawBox().
 */
RTEXTJPAPI RTextJPMetrics RTextJPMeasure(const char *text, float maxWidth, RTextJPStyle style);

/**
 * \brief 指定位置からテキストをそのまま描画します。
 *
 * 改行は処理しますが、自動折り返し・揃え・クリップは行いません。
 * 矩形に収めたい場合は RTextJPDrawBox() を使用してください。
 *
 * \par English
 * Draws at a position, honoring explicit newlines but without box layout.
 */
 ...
 * 「（」などを行末へ、「。」などを行頭へ置かない簡易禁則処理を行います。
 *
 * \warning style.clip がtrueのときraylibのシザーモードを開始・終了します。
 * 呼び出し側の BeginScissorMode() の内側では使用しないでください。
 *
 * \par English
 * Draws wrapped text inside a rectangle with alignment and optional kinsoku rules.
 */
RTEXTJPAPI void RTextJPDrawBox(const char *text, Rectangle bounds, RTextJPStyle style);

/**
 * \brief 簡易縦書きの必要サイズを測定します。
 * \param maxHeight 1列の高さ。0以下では改行まで1列です。
 * \par English
 * Measures simple vertical text. lineCount contains the number of columns.
 */
RTEXTJPAPI RTextJPMetrics RTextJPMeasureVertical(const char *text, float maxHeight, RTextJPStyle style);

/**
 * \brief UTF-8テキストを簡易縦書きで描画します。
 *
 * 文字を1マスずつ上から下へ並べ、いっぱいになると次の列へ進みます。
 * ラテン文字の回転、縦書き専用字形、句読点位置のOpenType置換は行いません。
 * 本格的な組版ではなく、ゲームUIや演出用の軽量機能です。
 *
 * \par English
 * Draws simple vertical columns; it does not apply OpenType vertical glyph substitution.
 */
RTEXTJPAPI void RTextJPDrawVertical(const char *text, Rectangle bounds, RTextJPStyle style,
                                RTextJPVerticalDirection direction);

/**
 * \brief 呼び出し側の固定長バッファで入力状態を初期化します。
 * \param input 初期化する状態。
 * \param buffer NUL終端済み、または空の書き込み可能な配列。
 * \param capacity bufferの全容量（終端NULを含むバイト数）。
 * 容量末尾で文字が途中になる場合や不正な並びがある場合は、直前の正しいUTF-8境界で切ります。
 * \par English
 * Initializes a fixed-buffer input state and keeps the longest valid UTF-8 prefix.
 */
RTEXTJPAPI void RTextJPInputInit(RTextJPInput *input, char *buffer, int capacity);

/**
 * \brief 入力欄の内容を安全に置き換えます。
 * \return 全文を格納できた場合はtrue。容量不足では文字境界で切り詰めてfalse。
 * \par English
 * Replaces the field text, truncating only at a UTF-8 boundary.
 */
RTEXTJPAPI bool RTextJPInputSetText(RTextJPInput *input, const char *text);

/**
 * \brief 現在のカーソル位置へUTF-8文字列を挿入します。
 *
 * 改行と制御文字は無視します。IMEやプラットフォーム固有UIで確定した文字列を
 * 渡すためにも使用できます。
 * \return 1文字以上挿入できた場合はtrue。
 * \par English
 * Inserts valid, printable UTF-8 at the cursor. Useful for an external IME bridge.
 */
RTEXTJPAPI bool RTextJPInputInsert(RTextJPInput *input, const char *text);

/**
 * \brief raylibのキーボード入力でアクティブな入力欄を更新します。
 *
 * GetCharPressed()、左右/Home/End、Backspace/Delete、Ctrl+Vを処理します。
 * \return 文字列が変更されたフレームだけtrue。
 *
 * \note IMEの変換中表示や候補ウィンドウは提供しません。GetCharPressed()で確定文字を
 * 受け取れるかはraylibのプラットフォームバックエンドに依存します。
 *
 * \par English
 * Updates the active field from raylib input and returns true when text changed.
 */
RTEXTJPAPI bool RTextJPInputUpdate(RTextJPInput *input);

/**
 * \brief 1行入力欄を描画します。
 *
 * クリック判定と input.active の切り替えは呼び出し側で行います。
 * 長い文字列はカーソルが見える位置まで自動的に横へずらして描画されます。
 *
 * \par English
 * Draws a single-line field. The caller owns hit testing and active-state changes.
 */
RTEXTJPAPI void RTextJPDrawInputBox(const RTextJPInput *input, Rectangle bounds,
                                RTextJPStyle textStyle, RTextJPInputStyle inputStyle,
 ...
 * Implementation
 *----------------------------------------------------------------------------------------*/
#if defined(RTEXTJP_IMPLEMENTATION)
#if !defined(RTEXTJP_IMPLEMENTATION_ONCE)
#define RTEXTJP_IMPLEMENTATION_ONCE
...
#ifndef RTEXTJP_MALLOC
    #define RTEXTJP_MALLOC(size) malloc(size)
#endif
#ifndef RTEXTJP_FREE
    #define RTEXTJP_FREE(ptr) free(ptr)
#endif

typedef struct RTextJPLineInternal {
    const char *begin;
    const char *end;
    const char *next;
    float width;
...
static bool rtextjp_is_continuation(unsigned char value)
{
    return (value & 0xC0u) == 0x80u;
}

static int rtextjp_decode_checked(const char *text, int *bytes, bool *valid)
{
    ...
    if ((text == NULL) || (s[0] == 0)) {
        ...
        if (valid != NULL) *valid = (text != NULL);
        return 0;
    }

    hasSecond = s[1] != 0;
    hasThird = hasSecond && (s[2] != 0);
    hasFourth = hasThird && (s[3] != 0);

    if (s[0] < 0x80u) {
        result = (int)s[0];
        size = 1;
        ok = true;
    } else if ((s[0] >= 0xC2u) && (s[0] <= 0xDFu) && hasSecond &&
               rtextjp_is_continuation(s[1])) {
        result = ((int)(s[0] & 0x1Fu) << 6) | (int)(s[1] & 0x3Fu);
        size = 2;
        ok = true;
    } else if ((s[0] == 0xE0u) && hasThird &&
               (s[1] >= 0xA0u) && (s[1] <= 0xBFu) &&
               rtextjp_is_continuation(s[2])) {
        result = ((int)(s[0] & 0x0Fu) << 12) | ((int)(s[1] & 0x3Fu) << 6) |
                 (int)(s[2] & 0x3Fu);
        size = 3;
        ok = true;
    } else if (((s[0] >= 0xE1u) && (s[0] <= 0xECu)) &&
               hasThird &&
               rtextjp_is_continuation(s[1]) && rtextjp_is_continuation(s[2])) {
        result = ((int)(s[0] & 0x0Fu) << 12) | ((int)(s[1] & 0x3Fu) << 6) |
                 (int)(s[2] & 0x3Fu);
        size = 3;
        ok = true;
    } else if ((s[0] == 0xEDu) && hasThird &&
               (s[1] >= 0x80u) && (s[1] <= 0x9Fu) &&
               rtextjp_is_continuation(s[2])) {
        result = ((int)(s[0] & 0x0Fu) << 12) | ((int)(s[1] & 0x3Fu) << 6) |
                 (int)(s[2] & 0x3Fu);
        ...
        size = 3;
        ok = true;
    } else if ((s[0] == 0xF0u) && hasFourth &&
               (s[1] >= 0x90u) && (s[1] <= 0xBFu) &&
               rtextjp_is_continuation(s[2]) && rtextjp_is_continuation(s[3])) {
        result = ((int)(s[0] & 0x07u) << 18) | ((int)(s[1] & 0x3Fu) << 12) |
        ...
        ok = true;
               ...
               hasFourth &&
               rtextjp_is_continuation(s[1]) && rtextjp_is_continuation(s[2]) &&
               rtextjp_is_continuation(s[3])) {
        result = ((int)(s[0] & 0x07u) << 18) | ((int)(s[1] & 0x3Fu) << 12) |
                 ((int)(s[2] & 0x3Fu) << 6) | (int)(s[3] & 0x3Fu);
        size = 4;
        ok = true;
    } else if ((s[0] == 0xF4u) && hasFourth &&
               (s[1] >= 0x80u) && (s[1] <= 0x8Fu) &&
               rtextjp_is_continuation(s[2]) && rtextjp_is_continuation(s[3])) {
        result = ((int)(s[0] & 0x07u) << 18) | ((int)(s[1] & 0x3Fu) << 12) |
                 ((int)(s[2] & 0x3Fu) << 6) | (int)(s[3] & 0x3Fu);
...
RTEXTJPAPI int RTextJPUtf8Decode(const char *text, int *bytes)
{
    return rtextjp_decode_checked(text, bytes, NULL);
}
...
RTEXTJPAPI bool RTextJPUtf8IsValid(const char *text)
{
    ...
    while (*cursor != '\0') {
        int bytes = 0;
        bool valid = false;
        (void)rtextjp_decode_checked(cursor, &bytes, &valid);
...
RTEXTJPAPI int RTextJPUtf8CodepointCount(const char *text)
{
    ...
    while (*cursor != '\0') {
        int bytes = 0;
        (void)RTextJPUtf8Decode(cursor, &bytes);
        cursor += (bytes > 0) ? bytes : 1;
        count++;
    }
    return count;
}
...
RTEXTJPAPI int RTextJPUtf8ByteIndex(const char *text, int codepointIndex)
{
    int index = 0;
    const char *cursor = text;
    if ((text == NULL) || (codepointIndex <= 0)) return 0;

    while ((*cursor != '\0') && (index < codepointIndex)) {
        int bytes = 0;
        (void)RTextJPUtf8Decode(cursor, &bytes);
        cursor += (bytes > 0) ? bytes : 1;
        index++;
...
RTEXTJPAPI RTextJPStyle RTextJPStyleDefault(Font font, float fontSize, Color color)
{
    RTextJPStyle style;
    style.font = font;
    style.fontSize = (fontSize > 0.0f) ? fontSize : 1.0f;
    style.spacing = 1.0f;
    style.lineSpacing = style.fontSize * 0.25f;
    style.color = color;
...
RTEXTJPAPI RTextJPInputStyle RTextJPInputStyleDefault(void)
{
    RTextJPInputStyle style;
    style.backgroundColor = (Color){ 250, 250, 250, 255 };
    style.borderColor = (Color){ 160, 160, 160, 255 };
    style.activeBorderColor = (Color){ 35, 120, 210, 255 };
    style.placeholderColor = (Color){ 145, 145, 145, 255 };
    style.cursorColor = (Color){ 20, 20, 20, 255 };
    style.padding = 8.0f;
    style.borderWidth = 2.0f;
    return style;
}

static bool rtextjp_codepoint_exists(const int *items, int count, int codepoint)
{
    int i = 0;
    for (i = 0; i < count; i++) {
        if (items[i] == codepoint) return true;
    }
    return false;
}

/* RTEXTJP_JIS_TABLES_BEGIN
 ...
 */
static const unsigned short rtextjp_jis_non_kanji[] = {
    /* ku 01 */
    ...
    0x2190, 0x2191, 0x2193, 0x3013, 0x2208, 0x220B, 0x2286, 0x2287, 0x2282, 0x2283,
    0x222A, 0x2229, 0x2227, 0x2228, 0x00AC, 0x21D2, 0x21D4, 0x2200, 0x2203, 0x2220,
    0x22A5, 0x2312, 0x2202, 0x2207, 0x2261, 0x2252, 0x226A, 0x226B, 0x221A, 0x223D,
    0x221D, 0x2235, 0x222B, 0x222C, 0x212B, 0x2030, 0x266F, 0x266D, 0x266A, 0x2020,
    0x2021, 0x00B6, 0x25EF,
    /* ku 03 */
    ...
    0xFF35, 0xFF36, 0xFF37, 0xFF38, 0xFF39, 0xFF3A, 0xFF41, 0xFF42, 0xFF43, 0xFF44,
...
static const unsigned short rtextjp_jis_level_1[] = {
    /* ku 16 */
...
static const unsigned short rtextjp_jis_level_2[] = {
    /* ku 48 */
...
/* RTEXTJP_JIS_TABLES_END */

static void rtextjp_append_codepoint(int *items, int capacity, int *count, int codepoint)
{
    if ((items == NULL) || (count == NULL) || (*count >= capacity)) return;
    if (!rtextjp_codepoint_exists(items, *count, codepoint)) items[(*count)++] = codepoint;
}

static void rtextjp_append_table(int *items, int capacity, int *count,
                               const unsigned short *table, int tableCount)
{
    int i = 0;
    for (i = 0; i < tableCount; i++) {
        rtextjp_append_codepoint(items, capacity, count, (int)table[i]);
    }
}

static bool rtextjp_increase_capacity(int *capacity, int addition)
{
    if ((capacity == NULL) || (addition < 0) || (*capacity > (INT_MAX - addition))) {
        return false;
    }
    *capacity += addition;
    return true;
...
static int *rtextjp_build_character_set(RTextJPCharacterSet characterSet,
                                      const char *customCharacters, int *resultCount)
{
    const int knownFlags = RTEXTJP_CHARACTER_SET_JIS_X_0208_KANA |
                           RTEXTJP_CHARACTER_SET_JIS_X_0208;
    int flags = ((int)characterSet) & knownFlags;
    int customCount = RTextJPUtf8CodepointCount(customCharacters);
    int capacity = 96;
    int count = 0;
    int *codepoints = NULL;
    int codepoint = 0;
    const char *cursor = customCharacters;
    int i = 0;

    if (resultCount != NULL) *resultCount = 0;
    if ((flags & RTEXTJP_CHARACTER_SET_JIS_X_0208_KANA) != 0) {
        if (!rtextjp_increase_capacity(&capacity, 169)) return NULL;
    }
    if ((flags & RTEXTJP_CHARACTER_SET_JIS_X_0208_NON_KANJI) != 0) {
        if (!rtextjp_increase_capacity(&capacity,
            (int)(sizeof(rtextjp_jis_non_kanji)/sizeof(rtextjp_jis_non_kanji[0])))) return NULL;
    }
    if ((flags & RTEXTJP_CHARACTER_SET_JIS_X_0208_LEVEL_1) != 0) {
        if (!rtextjp_increase_capacity(&capacity,
            (int)(sizeof(rtextjp_jis_level_1)/sizeof(rtextjp_jis_level_1[0])))) return NULL;
    }
    if ((flags & RTEXTJP_CHARACTER_SET_JIS_X_0208_LEVEL_2) != 0) {
        if (!rtextjp_increase_capacity(&capacity,
            (int)(sizeof(rtextjp_jis_level_2)/sizeof(rtextjp_jis_level_2[0])))) return NULL;
    }
    if (!rtextjp_increase_capacity(&capacity, customCount)) return NULL;

    codepoints = (int *)RTEXTJP_MALLOC((size_t)capacity * sizeof(int));
    if (codepoints == NULL) return NULL;

    for (codepoint = 0x20; codepoint <= 0x7E; codepoint++) codepoints[count++] = codepoint;
    codepoints[count++] = 0x3000;

    if ((flags & RTEXTJP_CHARACTER_SET_JIS_X_0208_KANA) != 0) {
        for (i = 0x3041; i <= 0x3093; i++) {
            rtextjp_append_codepoint(codepoints, capacity, &count, i);
        }
        for (i = 0x30A1; i <= 0x30F6; i++) {
            rtextjp_append_codepoint(codepoints, capacity, &count, i);
        }
    }
    if ((flags & RTEXTJP_CHARACTER_SET_JIS_X_0208_NON_KANJI) != 0) {
        rtextjp_append_table(codepoints, capacity, &count, rtextjp_jis_non_kanji,
            (int)(sizeof(rtextjp_jis_non_kanji)/sizeof(rtextjp_jis_non_kanji[0])));
    }
    if ((flags & RTEXTJP_CHARACTER_SET_JIS_X_0208_LEVEL_1) != 0) {
        rtextjp_append_table(codepoints, capacity, &count, rtextjp_jis_level_1,
            (int)(sizeof(rtextjp_jis_level_1)/sizeof(rtextjp_jis_level_1[0])));
    }
    if ((flags & RTEXTJP_CHARACTER_SET_JIS_X_0208_LEVEL_2) != 0) {
        rtextjp_append_table(codepoints, capacity, &count, rtextjp_jis_level_2,
            (int)(sizeof(rtextjp_jis_level_2)/sizeof(rtextjp_jis_level_2[0])));
    }

    while ((cursor != NULL) && (*cursor != '\0')) {
        int bytes = 0;
        bool valid = false;
        codepoint = rtextjp_decode_checked(cursor, &bytes, &valid);
        cursor += (bytes > 0) ? bytes : 1;

        if (!valid || (codepoint == '\n') || (codepoint == '\r') || (codepoint == '\t')) continue;
        rtextjp_append_codepoint(codepoints, capacity, &count, codepoint);
    }

    if (resultCount != NULL) *resultCount = count;
    return codepoints;
}
...
{
    int count = 0;
    int *codepoints = rtextjp_build_character_set(characterSet, customCharacters, &count);
                                            ...
                                            const char *customCharacters)
{
    ...
    if ((fileName == NULL) || (baseSize <= 0)) return emptyFont;
    codepoints = rtextjp_build_character_set(characterSet, customCharacters, &count);
    if (codepoints == NULL) return emptyFont;
    if (count <= 0) {
        RTEXTJP_FREE(codepoints);
        return emptyFont;
    }

    result = LoadFontEx(fileName, baseSize, codepoints, count);
    RTEXTJP_FREE(codepoints);
    return result;
}
...
{
    Font emptyFont = { 0 };
    if (characters == NULL) return emptyFont;
    return RTextJPLoadFontFromCharacterSet(fileName, baseSize,
                                         RTEXTJP_CHARACTER_SET_CUSTOM, characters);
}

static bool rtextjp_is_space(int codepoint)
{
    return (codepoint == 0x20) || (codepoint == 0x09) || (codepoint == 0x3000);
}

static bool rtextjp_is_cjk(int codepoint)
{
...
static bool rtextjp_prohibited_line_start(int codepoint)
{
    switch (codepoint) {
        case 0x0021: case 0x0029: case 0x002C: case 0x002E: case 0x003A: case 0x003B:
        case 0x003F: case 0x005D: case 0x007D:
        case 0x2019: case 0x201D: case 0x2025: case 0x2026:
        ...
        case 0x301C: case 0x301E: case 0x301F:
        case 0x3041: case 0x3043: case 0x3045: case 0x3047: case 0x3049:
        case 0x3063: case 0x3083: case 0x3085: case 0x3087: case 0x308E:
        case 0x3095: case 0x3096: case 0x30A1: case 0x30A3: case 0x30A5:
        case 0x30A7: case 0x30A9: case 0x30C3: case 0x30E3: case 0x30E5:
        case 0x30E7: case 0x30EE: case 0x30F5: case 0x30F6: case 0x30FB:
        case 0x30FC: case 0xFF01: case 0xFF09: case 0xFF0C: case 0xFF0E:
...
static float rtextjp_codepoint_width(Font font, int codepoint, float fontSize)
{
    float scale = 0.0f;
    int glyphIndex = 0;
    GlyphInfo glyph;
    ...
    if ((font.baseSize <= 0) || (fontSize <= 0.0f) || (font.glyphCount <= 0) ||
        (font.glyphs == NULL) || (font.recs == NULL)) return 0.0f;
    scale = fontSize/(float)font.baseSize;
    glyphIndex = GetGlyphIndex(font, codepoint);
    if ((glyphIndex < 0) || (glyphIndex >= font.glyphCount)) glyphIndex = 0;
    glyph = font.glyphs[glyphIndex];
    rec = font.recs[glyphIndex];
    if (glyph.advanceX != 0) return (float)glyph.advanceX*scale;
    return rec.width*scale;
}

static float rtextjp_visible_codepoint_width(RTextJPStyle style, int codepoint)
{
    if (codepoint == '\t') {
        return rtextjp_codepoint_width(style.font, 0x20, style.fontSize)*4.0f;
    }
    return rtextjp_codepoint_width(style.font, codepoint, style.fontSize);
}

static float rtextjp_measure_range(const char *begin, const char *end, RTextJPStyle style,
                                 int *codepointCount)
{
    float width = 0.0f;
    int count = 0;
    ...
    while ((cursor != NULL) && (cursor < end) && (*cursor != '\0')) {
        ...
        if (bytes <= 0) break;
        if (count > 0) width += style.spacing;
        width += rtextjp_visible_codepoint_width(style, codepoint);
        cursor += bytes;
        count++;
    }
    if (codepointCount != NULL) *codepointCount = count;
    return width;
}

static bool rtextjp_can_break_between(int left, int right, bool kinsoku)
{
    if (right == 0) return false;
    if (kinsoku && (rtextjp_prohibited_line_end(left) || rtextjp_prohibited_line_start(right))) {
        return false;
    }
    return rtextjp_is_cjk(left) || rtextjp_is_cjk(right);
}

static RTextJPLineInternal rtextjp_next_line(const char *start, float maxWidth, RTextJPStyle style)
{
    RTextJPLineInternal line;
    const char *cursor = start;
    const char *breakEnd = NULL;
    const char *breakNext = NULL;
    float width = 0.0f;
    ...
    while (*cursor != '\0') {
        ...
        int bytes = 0;
        int codepoint = RTextJPUtf8Decode(cursor, &bytes);
        float widthBefore = width;
        float addition = 0.0f;

        if (bytes <= 0) break;
        if ((codepoint == '\n') || (codepoint == '\r')) {
            const char *next = cursor + bytes;
            if ((codepoint == '\r') && (*next == '\n')) next++;
            line.end = cursor;
            line.next = next;
            line.width = width;
            line.codepointCount = count;
            line.hardBreak = true;
            return line;
        }

        addition = rtextjp_visible_codepoint_width(style, codepoint);
        if (count > 0) addition += style.spacing;

        if ((maxWidth > 0.0f) && (count > 0) && ((width + addition) > maxWidth)) {
            if (rtextjp_is_space(codepoint)) {
                const char *next = cursor + bytes;
                while (*next != '\0') {
                    int nextBytes = 0;
                    int nextCodepoint = RTextJPUtf8Decode(next, &nextBytes);
                    if ((nextBytes <= 0) || !rtextjp_is_space(nextCodepoint)) break;
                    next += nextBytes;
                }
                line.end = ((breakEnd != NULL) && (breakEnd > start)) ? breakEnd : codepointStart;
                line.next = next;
                line.width = ((breakEnd != NULL) && (breakEnd > start)) ? breakWidth : width;
                line.codepointCount = ((breakEnd != NULL) && (breakEnd > start)) ? breakCount : count;
                return line;
            }
            if ((breakEnd != NULL) && (breakEnd > start)) {
                line.end = breakEnd;
                line.next = breakNext;
                line.width = breakWidth;
                line.codepointCount = breakCount;
                return line;
            }
            if (!(style.kinsoku && rtextjp_prohibited_line_start(codepoint))) {
                line.end = codepointStart;
                line.next = codepointStart;
                line.width = width;
                line.codepointCount = count;
                return line;
            }
        ...
        if (rtextjp_is_space(codepoint)) {
            if ((breakEnd != NULL) && (breakNext == codepointStart)) {
                /* 連続空白は最初の空白からすべて次行の前で読み飛ばします。 */
                breakNext = cursor;
            } else {
                breakEnd = codepointStart;
                breakNext = cursor;
            ...
            int nextBytes = 0;
            int nextCodepoint = RTextJPUtf8Decode(cursor, &nextBytes);
            if (rtextjp_can_break_between(codepoint, nextCodepoint, style.kinsoku)) {
                breakEnd = cursor;
                breakNext = cursor;
                breakWidth = width;
                breakCount = count;
...
static void rtextjp_draw_range(const char *begin, const char *end, Vector2 position,
                             RTextJPStyle style)
{
    const char *cursor = begin;
    int index = 0;
    ...
    while ((cursor != NULL) && (cursor < end) && (*cursor != '\0')) {
        ...
        width = rtextjp_visible_codepoint_width(style, codepoint);
        if (codepoint != '\t') DrawTextCodepoint(style.font, codepoint, position,
...
RTEXTJPAPI RTextJPMetrics RTextJPMeasure(const char *text, float maxWidth, RTextJPStyle style)
{
    ...
    const char *cursor = text;
    float wrapWidth = style.wrap ? maxWidth : 0.0f;
    bool trailingEmptyLine = false;

    if ((text == NULL) || (*text == '\0')) return metrics;
    metrics.codepointCount = RTextJPUtf8CodepointCount(text);

    while (*cursor != '\0') {
        RTextJPLineInternal line = rtextjp_next_line(cursor, wrapWidth, style);
        if (line.width > metrics.size.x) metrics.size.x = line.width;
        metrics.lineCount++;
        trailingEmptyLine = line.hardBreak && (*line.next == '\0');
        if (line.next <= cursor) break;
        cursor = line.next;
    }

    if (trailingEmptyLine) metrics.lineCount++;
    if (metrics.lineCount > 0) {
        metrics.size.y = style.fontSize*(float)metrics.lineCount +
                         style.lineSpacing*(float)(metrics.lineCount - 1);
    }
    return metrics;
}
...
RTEXTJPAPI void RTextJPDraw(const char *text, Vector2 position, RTextJPStyle style)
{
    ...
    while (*cursor != '\0') {
        ...
        rtextjp_draw_range(line.begin, line.end, position, style);
        if (line.next <= cursor) break;
        cursor = line.next;
...
{
    if (alignment == RTEXTJP_ALIGN_CENTER) return bounds.x + (bounds.width - lineWidth)*0.5f;
    if (alignment == RTEXTJP_ALIGN_RIGHT) return bounds.x + bounds.width - lineWidth;
    return bounds.x;
}
...
RTEXTJPAPI void RTextJPDrawBox(const char *text, Rectangle bounds, RTextJPStyle style)
{
    ...
    const char *cursor = text;
    float y = bounds.y;
    float wrapWidth = style.wrap ? bounds.width : 0.0f;

    if ((text == NULL) || (*text == '\0') || (bounds.width <= 0.0f) ||
        (bounds.height <= 0.0f)) return;

    metrics = RTextJPMeasure(text, bounds.width, style);
    if (style.verticalAlignment == RTEXTJP_VALIGN_MIDDLE) {
        y += (bounds.height - metrics.size.y)*0.5f;
    } else if (style.verticalAlignment == RTEXTJP_VALIGN_BOTTOM) {
        y += bounds.height - metrics.size.y;
    }

    if (style.clip) {
        BeginScissorMode((int)floorf(bounds.x), (int)floorf(bounds.y),
    ...
    while (*cursor != '\0') {
        RTextJPLineInternal line = rtextjp_next_line(cursor, wrapWidth, style);
        Vector2 position;
        position.x = rtextjp_aligned_x(bounds, line.width, style.alignment);
        position.y = y;
        rtextjp_draw_range(line.begin, line.end, position, style);
...
RTEXTJPAPI RTextJPMetrics RTextJPMeasureVertical(const char *text, float maxHeight, RTextJPStyle style)
{
    RTextJPMetrics metrics = { 0 };
    const char *cursor = text;
    float rowAdvance = style.fontSize + style.lineSpacing;
    ...
    while (*cursor != '\0') {
        ...
        if ((maxHeight > 0.0f) && (rows > 0) &&
            ((currentHeight + rowAdvance) > (maxHeight + style.lineSpacing))) {
            if (currentHeight > largestHeight) largestHeight = currentHeight;
            metrics.lineCount++;
            currentHeight = 0.0f;
            rows = 0;
        ...
        if (rows > 0) currentHeight += style.lineSpacing;
        currentHeight += style.fontSize;
        rows++;
    }

    if (currentHeight > largestHeight) largestHeight = currentHeight;
    metrics.size.x = style.fontSize*(float)metrics.lineCount +
                     style.spacing*(float)(metrics.lineCount - 1);
    metrics.size.y = largestHeight;
    return metrics;
}
                                ...
                                RTextJPVerticalDirection direction)
{
    const char *cursor = text;
    float rowAdvance = style.fontSize + style.lineSpacing;
    float columnAdvance = style.fontSize + style.spacing;
    float x = bounds.x;
    float y = bounds.y;
    int row = 0;

    if ((text == NULL) || (bounds.width <= 0.0f) || (bounds.height <= 0.0f)) return;
    if (direction == RTEXTJP_VERTICAL_RIGHT_TO_LEFT) x += bounds.width - style.fontSize;
    ...
    while (*cursor != '\0') {
        ...
        if ((codepoint == '\n') || (codepoint == '\r')) {
            if ((codepoint == '\r') && (*cursor == '\n')) cursor++;
            x += (direction == RTEXTJP_VERTICAL_RIGHT_TO_LEFT) ? -columnAdvance : columnAdvance;
            y = bounds.y;
            row = 0;
            continue;
        }

        if ((row > 0) && ((y + style.fontSize) > (bounds.y + bounds.height))) {
            x += (direction == RTEXTJP_VERTICAL_RIGHT_TO_LEFT) ? -columnAdvance : columnAdvance;
            y = bounds.y;
            row = 0;
        }

        glyphWidth = rtextjp_visible_codepoint_width(style, codepoint);
        position.x = x + (style.fontSize - glyphWidth)*0.5f;
        position.y = y;
        if (codepoint != '\t') DrawTextCodepoint(style.font, codepoint, position,
                                                  style.fontSize, style.color);
        y += rowAdvance;
...
static int rtextjp_bounded_length(const char *text, int maximum)
{
    int length = 0;
...
RTEXTJPAPI void RTextJPInputInit(RTextJPInput *input, char *buffer, int capacity)
{
    ...
    input->length = rtextjp_bounded_length(buffer, capacity - 1);
    buffer[input->length] = '\0';

    while (validLength < input->length) {
        int bytes = 0;
        bool valid = false;
        (void)rtextjp_decode_checked(buffer + validLength, &bytes, &valid);
        if (!valid || (bytes <= 0) || ((validLength + bytes) > input->length)) break;
        validLength += bytes;
    }
...
static bool rtextjp_input_insert_codepoint(RTextJPInput *input, int codepoint)
{
    char encoded[5];
    int bytes = 0;
    if ((input == NULL) || (input->text == NULL) || (input->capacity <= 0)) return false;
    if ((codepoint < 0x20) || (codepoint == 0x7F)) return false;
    bytes = RTextJPUtf8Encode(codepoint, encoded);
    if ((bytes <= 0) || ((input->length + bytes) >= input->capacity)) return false;

    memmove(input->text + input->cursor + bytes, input->text + input->cursor,
            (size_t)(input->length - input->cursor + 1));
    memcpy(input->text + input->cursor, encoded, (size_t)bytes);
    input->cursor += bytes;
    input->length += bytes;
    return true;
}
...
RTEXTJPAPI bool RTextJPInputInsert(RTextJPInput *input, const char *text)
{
    ...
    while (*cursor != '\0') {
        int bytes = 0;
        bool valid = false;
        int codepoint = rtextjp_decode_checked(cursor, &bytes, &valid);
        cursor += (bytes > 0) ? bytes : 1;
        if (!valid) continue;
        if (rtextjp_input_insert_codepoint(input, codepoint)) changed = true;
    }
    return changed;
}
...
RTEXTJPAPI bool RTextJPInputSetText(RTextJPInput *input, const char *text)
{
    const char *cursor = text;
    bool complete = true;
    if ((input == NULL) || (input->text == NULL) || (text == NULL)) return false;
    ...
    while (*cursor != '\0') {
        ...
        if (!rtextjp_input_insert_codepoint(input, codepoint)) {
            complete = false;
            break;
        }
    }
    return complete && (*cursor == '\0');
}

static int rtextjp_previous_boundary(const RTextJPInput *input, int position)
{
    if ((input == NULL) || (input->text == NULL) || (position <= 0)) return 0;
    position--;
    while ((position > 0) && rtextjp_is_continuation((unsigned char)input->text[position])) {
        position--;
    }
    return position;
}

static int rtextjp_next_boundary(const RTextJPInput *input, int position)
{
    int bytes = 0;
    if ((input == NULL) || (input->text == NULL) || (position >= input->length)) {
        return (input != NULL) ? input->length : 0;
    }
    (void)RTextJPUtf8Decode(input->text + position, &bytes);
    if (bytes <= 0) bytes = 1;
    position += bytes;
...
static bool rtextjp_key_pressed_or_repeat(int key)
{
...
RTEXTJPAPI bool RTextJPInputUpdate(RTextJPInput *input)
{
    ...
    if ((input == NULL) || (input->text == NULL) || !input->active) return false;
    control = IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_RIGHT_CONTROL);

    if (control && IsKeyPressed(KEY_V)) {
        const char *clipboard = GetClipboardText();
        if (clipboard != NULL) changed = RTextJPInputInsert(input, clipboard) || changed;
    }

    while ((codepoint = GetCharPressed()) > 0) {
        if (!control && rtextjp_input_insert_codepoint(input, codepoint)) changed = true;
    }

    if (rtextjp_key_pressed_or_repeat(KEY_LEFT)) {
        input->cursor = rtextjp_previous_boundary(input, input->cursor);
    }
    if (rtextjp_key_pressed_or_repeat(KEY_RIGHT)) {
        input->cursor = rtextjp_next_boundary(input, input->cursor);
    }
    if (IsKeyPressed(KEY_HOME)) input->cursor = 0;
    if (IsKeyPressed(KEY_END)) input->cursor = input->length;

    if (rtextjp_key_pressed_or_repeat(KEY_BACKSPACE) && (input->cursor > 0)) {
        int previous = rtextjp_previous_boundary(input, input->cursor);
        memmove(input->text + previous, input->text + input->cursor,
                (size_t)(input->length - input->cursor + 1));
        input->length -= input->cursor - previous;
        input->cursor = previous;
        changed = true;
    }
    if (rtextjp_key_pressed_or_repeat(KEY_DELETE) && (input->cursor < input->length)) {
        int next = rtextjp_next_boundary(input, input->cursor);
        memmove(input->text + input->cursor, input->text + next,
                (size_t)(input->length - next + 1));
        input->length -= next - input->cursor;
        changed = true;
    }
                                ...
                                const char *placeholder)
{
    ...
    if ((input == NULL) || (input->text == NULL) || (bounds.width <= 0.0f) ||
        (bounds.height <= 0.0f)) return;

    DrawRectangleRec(bounds, inputStyle.backgroundColor);
    DrawRectangleLinesEx(bounds, inputStyle.borderWidth,
                         input->active ? inputStyle.activeBorderColor : inputStyle.borderColor);

    inner.x = bounds.x + inputStyle.padding;
    inner.y = bounds.y + inputStyle.padding;
    inner.width = bounds.width - inputStyle.padding*2.0f;
    inner.height = bounds.height - inputStyle.padding*2.0f;
    if ((inner.width <= 0.0f) || (inner.height <= 0.0f)) return;

    cursorWidth = rtextjp_measure_range(input->text, input->text + input->cursor,
                                      textStyle, NULL);
    fullWidth = rtextjp_measure_range(input->text, input->text + input->length,
                                    textStyle, NULL);
    if (cursorWidth > inner.width) offsetX = inner.width - cursorWidth;
    if ((input->cursor == input->length) && (fullWidth + offsetX < inner.width)) {
        offsetX = 0.0f;
    }

    textY = inner.y + (inner.height - textStyle.fontSize)*0.5f;
    displayText = input->text;
    if ((*displayText == '\0') && (placeholder != NULL)) {
        displayText = placeholder;
        textStyle.color = inputStyle.placeholderColor;
        offsetX = 0.0f;
    }

    BeginScissorMode((int)floorf(inner.x), (int)floorf(inner.y),
                     (int)ceilf(inner.width), (int)ceilf(inner.height));
    if (*displayText != '\0') {
        Vector2 textPosition = { inner.x + offsetX, textY };
        RTextJPDraw(displayText, textPosition, textStyle);
    }

    if (input->active && (((int)(GetTime()*2.0)) % 2 == 0)) {
        float cursorX = inner.x + offsetX + cursorWidth;
        DrawRectangle((int)cursorX, (int)textY, 2, (int)textStyle.fontSize,
                      inputStyle.cursorColor);
    }
    EndScissorMode();
}

#endif /* RTEXTJP_IMPLEMENTATION_ONCE */
#endif /* RTEXTJP_IMPLEMENTATION */
